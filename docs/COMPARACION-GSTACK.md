...
| /qa con navegador | ✅ `navegador.mjs` | `[IMPLEMENTADA]` motor primario de `/qa`: navegar/snapshot/click/escribir/texto/screenshot con eval end-to-end (Chromium real + servidor HTTP local) cuando Playwright está instalado, omisión clara si no. Playwright se declara como dependencia de desarrollo reproducible; no se embebe en el runtime del núcleo — mismo patrón de carga dinámica que graphify en `/grafo` |
| Import de cookies del navegador real | ✅ `[IMPLEMENTADA]` | `cookies.mjs` + acción `perfil` en `navegador.mjs` — NO lee el almacén cifrado del navegador (sin SQLite, sin DPAPI, sin riesgos dual-use). Usa Playwright `storageState`: el usuario autentica UNA vez en Chromium visible, `cookies.mjs guardar <dominio>` exporta storageState a `.fabrica/auth/<dominio>.json`, y `navegador.mjs ejecutar '[{accion:"perfil",dominio:"..."}]'` lo inyecta en las sesiones siguientes. Cross-platform, solo el dominio que el usuario pide (no todas sus cookies), cero deps extra. Skill: `/autenticar`. **Nota (spike 2026-07-19):** el camino DPAPI+SQLite (leer el almacén cifrado real) se probó factible con evidencia — `node:sqlite` existe en Node 24, DPAPI descifra la master key vía PowerShell, AES-256-GCM v10 descifra en Node built-in — pero se descartó deliberadamente: Chrome mantiene lock EXCLUSIVO del archivo mientras corre (ni copy, ni FileShare.Read, ni esentutl lo evaden; VSS exige admin), es Windows-only y más invasivo. storageState es superior para el objetivo. Detalle en `.fabrica/problemas/cookies-navegador.md` |
...
| Evals tier 2 (E2E con sesión real) | ✅ `evals/tier2.mjs` | `[IMPLEMENTADA]` sin LLM (assertions estructurales, no juzgadas — eso es tier 3): sesión de sprint completa simulada (pensar→retro, ~15 subprocesos reales) verificando integridad de estado/memoria/git entre `estado.mjs`, `checkpoint.mjs`, `grafo.mjs`, `pruebas.mjs`. Job separado de CI. El propio test encontró un gap real de integración (`pruebas.mjs afectadas` sin rango pierde el cambio si un checkpoint ya lo commiteó) — documentado en `/pruebas-afectadas` |
| Evals tier 3 (LLM-juez) | ✅ `[IMPLEMENTADA]` | `nucleo/juez.mjs` — invoca el CLI del LLM disponible (`claude`, `gemini`) dinámicamente (cero SDK, cero keys embebidas, mismo patrón que Playwright). Job MANUAL, no CI — el usuario decide cuándo pagar. Rubricas estructuradas por skill (5 criterios específicos, 0-2 puntos cada uno) para consistencia. Resultado guardado en `.fabrica/juez/` para auditoría. |
...
| Sync de memoria entre máquinas (repo git privado) | ✅ `[IMPLEMENTADA]` | `nucleo/sync.mjs` — push/pull de `.fabrica/` a un repo git privado. `mergeJsonl` hace merge append-only con dedup por LÍNEA completa (no por `id`: memoria.jsonl no tiene id, dedup por id la duplicaría) — idempotente, verificado por eval. Escaneo de secretos (`secretos.mjs`) ANTES de push. El usuario configura el repo con `sync.mjs configurar <url>`. **Auditoría 2026-07-19 encontró y corrigió 3 bugs reales que la eval-copia no veía:** `appendFileSync` usado sin importar (crash en el primer merge), el escáner de secretos leía una propiedad inexistente (`.encontrados` vs `.hallazgos` → nunca redactaba, credenciales al repo en claro), y el dedup por id duplicaba memoria. Eval reescrita para probar el código real. |
...
5. **Runtime mínimo reproducible** (Node puro) — sin dependencias externas de runtime; Playwright queda fijado como dependencia de desarrollo para QA/browser.
